...
from typing import List, Union, Optional, Any, Iterable, NoReturn, Literal
...
class Auth:
    ...
    async def get_premium_owning(self) -> bool:
        ...
        async with self.session.get(
                f"https://premiumfeatures.roblox.com/v1/users/{self.user_id}/validate-membership") as response:
...
class Item:
    def __init__(
        ...
        self._price_to_sell = offered_price
    ...
    @property
    def price_to_sell(self):
        return self._price_to_sell

    @price_to_sell.setter
    def price_to_sell(self, new: int):
        self._price_to_sell = min_sale_price(new)
    ...
    def add_collectible(
        ...
        col = self.collectibles.get(serial)
        ...
        if col is None and serial is not None:
            ...
            self.collectibles.update({serial: new})
    ...
    async def sell_collectibles(
        ...
        if [col for col in self.collectibles.values() if None in (col.auth, col.item_id, col.instance_id, col.product_id)]:
            await self.fetch_collectibles()

        sold_amount = 0
        price_to_sell = (price or self._price_to_sell)

        for col in self.collectibles.values():
            tries = 0
            
            if col.sale_price == price_to_sell:
                if log: display_skipping(f"This collectible is already on sale for the same price {GRAY_COLOR}(#{col.serial}){Color.white}")
                continue
            elif col.on_sale and skip_on_sale:
                if log: display_skipping(f"This collectible is already on sale {GRAY_COLOR}(#{col.serial}){Color.white}")
                continue
            ...
            while True:
                ...
                match status:
                    case 200:
                        if log: display_success(f"Successfully sold for ${GRAY_COLOR}{price_to_sell} "
                                                f"(#{col.serial}){Color(255, 255, 255)}")

                        sold_amount += 1
                        break
                    case 429:
                        if log: display_error("You got ratelimited! Trying again in 30 seconds...")

                        tries += 1
                        await asyncio.sleep(30)
                    case 403:
                        if log: display_error("Your token got updated. Getting a new one...")
                        await self.auth.update_csrf_token()
                    case 412:
                        if log: display_skipping("Item is not resable. Skipping it")
                        return None
                    case _:
                        if log: display_error(f"Failed to sell limited ({status})")
    ...
    async def fetch_resales(self,
        ...
        async with self.auth.session.get(
                f"https://apis.roblox.com/marketplace-sales/v1/item/{self.item_id}/resellers?limit=99999",
                ssl=False
        ) as response:
            try:
            ...
            if save_resales and not self.resales:
                for resale in data:
                    ...
                    data = {
                        "lowest_resale_price": lrp,
                        "serial": resale["serialNumber"],
                        "seller_id": seller["sellerId"],
                        "seller_name": seller["name"]
                    }
                    self.resales.update({lrp: data})

            if save_lrp and self.lowest_resale_price is None:
