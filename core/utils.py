...
class Item:
    ...
    def add_collectible(self, col: Collectible) -> None:
        self._collectibles.update({col.serial: col})
    ...
    async def sell_collectibles(
        ...
        if [col for col in self._collectibles.values() if None in (col.auth, col.item_id, col.instance_id, col.product_id)]:
