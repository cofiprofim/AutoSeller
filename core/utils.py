...
class FileSync(set):
    def __init__(self, filename: Union[str, PathLike]) -> None:
        ...
        if not isinstance(items, list):
            return Display.exception("Invalid format type provided")
...
class Collectible:
    __slots__ = ("_serial", "_on_sale", "_sale_price",
                 "_item_id", "_instance_id", "_product_id",
                 "skip_on_sale", "auth")
    ...
    on_sale = IgnoreNew()
    sale_price = IgnoreNew()
    item_id = IgnoreNew()
    instance_id = IgnoreNew()
    product_id = IgnoreNew()
...
class Item:
    # __slots__ = ("_id", "name", "__link",
    #              "price", "thumbnail", "quantity",
    #              "item_id", "_creator_id", "creator_name", "_creator_type"
    #              "__creator_link", "price_to_sell", "lowest_resale_price",
    #              "recent_average_price", "latest_sale", "has_resales",
    #              "_collectibles", "resales", "sales", "auth")
    ...
    def creator_type(self, new: Literal["Group", "User"]):
        ...
        defined_link = f"groups/{self._creator_id}" if self._creator_type == "Group" else f"users/{self._creator_id}/profile"
        self.__creator_link = f"https://www.roblox.com/{defined_link}"
    ...
    @staticmethod
    def __define_status(value: str, status: Union[bool, None], name: str):
        if status is True:
            return f"{value:,}"
    ...
    def define_lowest_resale_price(self) -> str:
        return self.__define_status(self.lowest_resale_price,
                                    self.has_resales,
                                    "resales")
    
    def define_recent_average_price(self) -> str:
        return self.__define_status(self.recent_average_price,
                                    self.has_sales,
                                    "sales")
    
    def define_latest_sale(self) -> str:
        return self.__define_status(self.latest_sale,
                                    self.has_sales,
                                    "sales")
    ...
    async def sell_collectibles(
        ...
        for col in self._collectibles.values():
            ...
            if col.sale_price == price_to_sell:
                ...
                if skip_on_sale:
                    if log: Display.skipping(f"This collectible is already on sale {GRAY_COLOR}(#{col.serial}){Color.white}")
                    continue
                elif skip_if_cheapest and self.lowest_resale_price == col.sale_price:
                    if log: Display.skipping(f"You are already selling this collectible for the cheapest price {GRAY_COLOR}(#{col.serial}){Color.white}")
                    continue
