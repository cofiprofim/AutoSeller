...
from typing import Optional, NoReturn, Mapping, List
...
async def display_done(text: str) -> NoReturn:
    timestamp = get_current_time()
    await ainput(f"{Color(190, 190, 190)}{timestamp} > {Color(163, 133, 0)}DONE{Color.white} | {text}")
    os.system("pause" if os.name == "nt" else "read -p \"Press any key to continue . . .\"")
    os._exit(0)

async def display_new(text: str) -> None:
    timestamp = get_current_time()
    await ainput(f"{Color(190, 190, 190)}{timestamp} > {Color(163, 133, 0)}NEW{Color.white} | {text}")
    os.system("pause" if os.name == "nt" else "read -p \"Press any key to continue . . .\"")
    os._exit(0)

def display_skipping(text: str, end: str = "\n") -> None:
    timestamp = get_current_time()
    print(f"{Color(190, 190, 190)}{timestamp} > {Color(110, 110, 110)}SKIPPING{Color.white} | {text}", end=end)
