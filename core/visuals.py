...
def print_centered(text: str, color: Optional[Color] = None, end: str = "\n") -> None:
...
def display_main() -> None:
    print_centered(TITLE, MAIN_COLOR)
    print_centered(SIGNATURE, ACCENT_COLOR, end="\n\n")
...
def display_sections(data: Mapping[str, Mapping[str, str]]) -> None:
    ...
    for section, values in data.items():
        print_centered(f">{GRAY_COLOR}>{Color.white} {section} {GRAY_COLOR}<{Color.white}<", Color.white, "\n\n")

        for name, value in values.items():
            name_indent = " " * (longest_name - len(str(name)))
            value_indent = " " * (longest_value - len(str(value)))

            print_centered(f"{GRAY_COLOR}> {name}{name_indent} :{Color.white} {value}{value_indent}")
