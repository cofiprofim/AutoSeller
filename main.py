...
try:
    ...
    if install:
        ...
        os.system("pip install aiohttp tqdm rgbprint discord.py aioconsole")
...
class AutoSeller:
    ...
    async def start(self):
        if self.buy_webhook_enabled and (not self.buy_webhook_url.startswith("https://discord.com/api/webhooks/")
                                           or not is_webhook_exists(self.buy_webhook_url)):
            return display_exception("Invalid on buy webhook url provided")
                                            ...
                                            or not is_webhook_exists(self.sale_webhook_url)):
        ...
        if self.keep_copy != 0:
            for ind, col in enumerate(self.items):
    ...
    async def start_buy_checking(self):
        async with aiohttp.ClientSession(
            ...
            while True:
                ...
                users_thumbnails = await get_users_thumbnails([str(user["id"]) for user in sales["agent"]], self.auth)
                ...
                for sale, user_thumbnail in zip(sales, users_thumbnails):
                    ...
                    if item is not None:
                        ...
                        await session.post(self.buy_webhook_url, json=embed)
    ...
    async def update_console(self) -> str:
        ...
        data = {
            ...
            "Current Item": {
                ...
                "Lowest Price": f"{self.current_asset.lowest_resale_price:,}" if self.current_asset.has_resales else "No Resales",
    ...
    async def send_sale_webhook(self, col: Item) -> None:
        embed = {
            "content": self.user_to_ping,
            "embeds": [{
