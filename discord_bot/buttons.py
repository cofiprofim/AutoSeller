...
class ControlPanel(View):
    ...
    async def sell(self, interaction: discord.Interaction, _: discord.ui.Button):
        ...
        try:
            sold_amount = await self.seller.current_asset.sell_collectibles(skip_on_sale=self.seller.hide_on_sale, log=False)
            if sold_amount is None:
                ...
                if self.seller.sale_webhook_enabled and sold_amount > 0:
                    asyncio.create_task(self.seller.send_sale_webhook(self.seller.current_asset, sold_amount))
