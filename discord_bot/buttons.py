...
class ControlPanel(View):
    ...
    def make_embed(self) -> discord.Embed:
        ...
        embed.add_field(name=f"Lowest Resale Price",
                        value=(f"`{self.seller.current_item.lowest_resale_price:,}`" if self.seller.current_item.has_resales is True
                               else "`No Resales`" if self.seller.current_item.has_resales is False
                               else "`Failed to Fetch`"),
                        inline=True)
        
        embed.add_field(name=f"Latest Sale",
                        value=(f"`{self.seller.current_item.latest_sale:,}`" if self.seller.current_item.has_sales is True
                               else "`No Sales`" if self.seller.current_item.has_sales is False
                               else "`Failed to Fetch`"),
                        inline=True)
        
        embed.add_field(name=f"RAP",
                        value=(f"`{self.seller.current_item.recent_average_price:,}`" if self.seller.current_item.has_sales is True
                               else "`No Sales`" if self.seller.current_item.has_sales is False
                               else "`Failed to Fetch`"),
                        inline=True)
            ...
            with self.seller.selling:
                try:
                    sold_amount = await self.seller.current_item.sell_collectibles(
                        ...
                        skip_if_cheapest=self.seller.skip_if_cheapest, log=False)
