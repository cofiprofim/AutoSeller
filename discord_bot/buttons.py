...
class ControlPanel(View):
    ...
    def make_embed(self) -> discord.Embed:
        ...
        embed.add_field(name=f"Lowest Resale Price", value="`No Resales`" if item.has_resales is False else f"`{item.lowest_resale_price:,}`", inline=True)
        embed.add_field(name=f"Latest Sale", value="`No Sales`" if item.has_sales is False else f"`{item.latest_sale:,}`", inline=True)
    ...
    async def sell(self, interaction: discord.Interaction, _: discord.ui.Button):
        ...
        try:
            sold_amount = await self.seller.current_item.sell_collectibles(skip_on_sale=self.seller.hide_on_sale, log=False)
            if sold_amount is None:
                ...
                if self.seller.sale_webhook_enabled and sold_amount > 0:
